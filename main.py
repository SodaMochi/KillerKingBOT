...
# エラーが起きた時は開発者のdiscordに報告
CHANNEL_FOR_REPORT = 1215209976805720124
FINAL_UPDATE = '2024-03-31'
...
class Role:
    ...
    async def UseAbility(self,player,game):
        if role_data[self.name]['ability_cooldown']>game._time_in_game:
...
class Player:
    ...
    async def PrintHelpMessage(self,game):
        res = {"あなたの名前":self.player_name,"ゲーム経過時間":f'{int(game._time_in_game)}分'}
    ...
    async def ReceiveMessage(self,sender_role:str,content:str,is_reply:bool=False):
        ...
        if is_reply:
            ...
            if sender_role not in self.replyable_roles: #土岐がジョーカーに何回も送った時用
...
class Ace(Role):
    ...
    async def UseAbility(self,player:Player,game):
        ...
        try:
            await super().UseAbility(player,game)
            if self.name!="エース": raise Exception('役職が変更されました')
        except Exception as e:
            await SendError(player.channel,e)
            return
...
class Club3(Role):
    ...
    async def UseAbility(self,player:Player,game):
        ...
        try:
            await super().UseAbility(player,game)
            if self.name!="クラブの３": raise Exception('役職が変更されました')
        except Exception as e:
            await SendError(player.channel,e)
            return

        # Swap
        joker_escape = game.Roles['ジョーカー'][0].role.escape_condition
        ...
        for player in game.Players.values():
            ...
            if player.role.name in game.Roles:
                game.Roles[player.role.name].append(player)
            else:
                game.Roles[player.role.name] = [player]
        game.Roles['ジョーカー'][0].role.escape_condition = joker_escape
...
class Queen(Role):
    ...
    async def UseAbility(self, player:Player, game):
        ...
        try:
            await super().UseAbility(player,game)
            if self.name!="クイーン": raise Exception('役職が変更されました')
        except Exception as e:
            await SendError(player.channel,e)
            return
        embed = discord.Embed(title=f'{res}の情報')
        embed.add_field(name='役職',value=game.Players[res].role.name,inline=False)
        if game.Players[res].role.name=='ジョーカー': embed.add_field(name='脱出条件',value=game.Players[res].role.escape_condition,inline=False)
...
async def SendSystemMessage(textchannel:discord.TextChannel,description='',headline='',content='',mention:str=None):
    if not textchannel: return #チャンネルが設定されていない
...
class Game:
    def __init__(self,loby:discord.TextChannel):
        ...
        # "ゲーム開始前" -> "ゲーム進行中" -> "ゲーム終了"　+ "中断"
        self.phase = "ゲーム開始前"
        # 更新するときはUpdateTimeを呼ぶ
        self._last_update_time:datetime.datetime = datetime.datetime.now()
        self._time_in_game:float = 0 #表示するときはint
    ...
    def Save(self):
        ...
        save["time_in_game"] = self._time_in_game
    ...
    def Load(self,guild_data:json):
        ...
        self._time_in_game = guild_data["time_in_game"]
    ...
    # 時刻更新、時間トリガーイベントを発動する
    async def UpdateTime(self):
        dt = (datetime.datetime.now() - self._last_update_time).total_seconds() / 60 #経過分数
        self._last_update_time = datetime.datetime.now()
        # 帝のメッセージ
        for trigger in [10,15,20,30]:
            if self._time_in_game < trigger <= self._time_in_game + dt:
                await self.Players['帝秀一'].channel.send(embed=discord.Embed(description=text_data['Mikado_Memory'][str(trigger)]))
        # 終了30分前の能力解放
        if self._time_in_game < 90 <= self._time_in_game + dt:
        ...
        if self._time_in_game < 120 <= self._time_in_game + dt:
        ...
        self._time_in_game += dt

    # 進行中に呼ぶと、再開する
    async def StartGame(self):
        if self.phase=='中断':
            await SendSystemMessage(self.loby,f'ゲームを再開しました\n経過時間:{int(self._time_in_game)}')
            self.phase = 'ゲーム進行中'
            self._last_update_time = datetime.datetime.now()
        elif self.phase=='ゲーム終了':
            msg = await SendSystemMessage(self.admin,'ゲームは既に終了しています。新規ゲームを始める場合は!deleteを入力してください。ゲームを終了前に戻す場合は「yes」を入力してください')
            try:
    ...
    async def EndGame(self):
        if self._time_in_game<120:
            msg = await SendSystemMessage(self.admin,f'現在ゲーム開始から{int(self._time_in_game)}分です。\n本当にゲームを終了させる場合は「yes」を送信してください')
            try:
    ...
    async def PrintAdminHelp(self,channel:discord.TextChannel=None):
        ...
        pre_game = {"allset":"すべてのチャンネル・ロールを作成する",
                    "start":"デスゲームを開始する"}
        rest = {"start":"ゲームを再開する"}
        after_game = {"start":"ゲームを進行中に戻す",
                      "delete":"データを削除し、新規ゲームデータを作成する",
                      "key":"プレイヤーに脱出パスワードを送信する"}
        embed = discord.Embed(title='Help')
        embed.add_field(name='現在の状況',value=self.phase)
        embed.add_field(name='ゲーム経過時間',value=format(self._time_in_game,'.2f'))
        ...
        elif self.phase=='ゲーム開始前': embed.add_field(name='ゲーム開始前コマンド',value='\n'.join([f'`!{key}    {value}`'for key,value in pre_game.items()]),inline=False)
        elif self.phase=='中断': embed.add_field(name='休憩中コマンド',value='\n'.join([f'`!{key}    {value}`'for key,value in rest.items()]),inline=False)
        elif self.phase=='ゲーム終了': embed.add_field(name='ゲーム終了後コマンド',value='\n'.join([f'`!{key}    {value}`'for key,value in after_game.items()]),inline=False)
    ...
    ''' tasks.loopがRepl.itのReserved VM下で機能しないようなので、UpdateTimeに統合
    ...
    async def Interpret(self,message:discord.Message):
        ...
        elif cmd=='help' and self.IsChannelReady()!=True:
            await self.PrintAdminHelp(message.channel)
            return
        elif cmd=='stop' and self.phase=='ゲーム進行中':
            self.phase = '中断'
            await SendSystemMessage(self.loby,'ゲームを中断しました\n再開するときはadminで「!start」を入力してください')
        elif cmd=="start" and message.channel!=self.admin:
            await SendError(message.channel,"adminチャンネルを「!set」で設定し、adminで「!start」してください")
        ...
        # adminコマンド
        if author=='admin':
            if cmd=='help': await self.PrintAdminHelp(message.channel)
        ...
        # プレイヤー
        if self.phase=="ゲーム開始前" and cmd=="help":
            await SendSystemMessage(message.channel,headline="ゲーム開始前です",content=f"あなたの名前:{author.player_name}")
        elif self.phase=="ゲーム終了":
            await SendSystemMessage(message.channel,headline="ゲームは終了しました",content=f"あなたの名前:{author.player_name}")
    ...
    async def SetChannel(self,channel:discord.TextChannel):
        ...
        await SendSystemMessage(channel,"チャンネル名を入力してください(ゲームマスター用は「admin」、ロビーは「loby」、その他はプレイヤー名を入力)")
        try:
            res = await WaitForResponse(channel)
        except:
    ...
    async def SetAllChannel(self,message:discord.Message):
        ...
        #権限チェック
        lack = []
        if not message.guild.me.guild_permissions.manage_channels: lack.append("チャンネルの管理")
        if not message.guild.me.guild_permissions.manage_roles: lack.append("ロールの管理")
        if lack:
            await SendError(message.channel,f"{','.join(lack)}の権限が不足しています")
            return
        ...
        for player_name in player_data:
            ...
            if player_name=="海山月正": #GM_KillerKing ロールを適用（ネタバレ防止）
                self.Players[player_name].channel = await message.guild.create_text_channel(name=f'{player_name}のスマホ',
                                                            category=message.channel.category,overwrites=admin_overwrites)
                continue
...
class MessageInputForm(View):
    ...
    async def button_callback(self,interaction:discord.Interaction,button:Button):
        ...
        try: 
            ...
            return
        # 実行
        if self.address=='帝秀一': #プレイヤーがジョーカーであり、帝秀一からのメッセージに返信する場合
            await self.game.Players['帝秀一'].ReceiveMessage('不明な宛先',self.content,self.is_reply)
...
class ViewForJoker(View):
    ...
    async def callback(self,interaction:Interaction,select:Select):
        ...
        await interaction.response.edit_message(content=f'{res}をコピーしました',embed=None,view=None)
...
class ViewForPassword(View):
    ...
    async def callback(self,interaction:Interaction,select:Select):
        ...
        await interaction.response.edit_message(embed=discord.Embed(description=f'{select.values[0]}にパスワード{d[select.values[0]]}を送信しました'),view=None)
    ...
    with open('save_data.json') as f:
        ...
        if str(message.guild.id) in data.keys():
            ...
            await SendSystemMessage(game.loby,headline="新規ゲームデータを作成しました",
                    content=f'最終アップデート ({FINAL_UPDATE}) 以前のデータから再開する場合は、!setでチャンネルの再設定、(ゲーム途中なら!changeで再設定)をお願いします')
            game.Save()
    return game
...
async def on_ready():
    ...
    #await client.get_guild(GUILD_FOR_REPORT).get_channel(CHANNEL_FOR_REPORT).send("ループを再開しました")
    #await client.get_channel(CHANNEL_FOR_REPORT).send("ループを再開しました")
    #loop.start() # 時間計測ループ
...
async def on_message(message:discord.Message):
    ...
    if message.channel.id==CHANNEL_FOR_REPORT: return
    # サーバーの認証
    game = await VerifyGuild(message)
    # コマンドの解釈・実行
    try:
        if game.phase=='ゲーム進行中':
            await game.UpdateTime() #時間を更新 (tasks.loopが使えないため、コマンドを呼ぶたびに時間を更新)
        await game.Interpret(message)
    except Exception as e:
        exception_type, exception_object, exception_traceback = sys.exc_info()
        embed = discord.Embed(title="ERROR REPORT")
        embed.add_field(name="object",value=exception_object)
        traceback_text = traceback.format_exc()
        if len(traceback_text)>1000: traceback_text = traceback_text[-1000:]
        embed.add_field(name="traceback",value=traceback_text,inline=False)
        embed.add_field(name='コマンド',value=message.content,inline=False)
        embed.add_field(name='ギルド',value=message.guild.id)
        embed.add_field(name='チャンネル',value=message.channel.name)

        await client.get_channel(CHANNEL_FOR_REPORT).send(embed=embed)
